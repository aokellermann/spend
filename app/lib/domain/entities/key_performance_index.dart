...
class KeyPerformanceIndexResponse {
  ...
  KeyPerformanceIndexResponse.fromJson(
      ...
      required KPICategory category,
      int? position}) {
    assert(!(route == NavigationRoute.overview && position == null),
        'Position must be provided for overview route');

    results = [];
    result = null;
    List<KeyPerformanceIndex> defaultKPIs =
        KPIUtils.getDefaultKPIsByRoute(route);
    int index = 0;
    ...
    for (var k in json.entries) {
      ...
      results.add(KeyPerformanceIndex.fromJson(
          json: k.value,
          name: k.key,
          route: route,
          category: category,
