...
class PasswordService extends ChangeNotifier {
  PasswordService() {
    ...
    passwordRepeatController = TextEditingController()
      ..addListener(_passwordRepeatListener);
    emailErrorText = null;
  ...
  Future<void> resetPassword(BuildContext context) async {
    if (!AuthUtils().isValidEmail(emailController.text)) {
      emailErrorText =
          Translations.of(context)!.text('textfield.error.invalid-email');
      notifyListeners();
    } else {
        ...
        if (success) {
          ...
          FlashMessageFactory.showFlashMessage(
              ...
              title: Translations.of(context)!
                  .text('snackbar.password-reset.failure'));
        }
      });
  ...
  Future<void> setNewPassword(String code, BuildContext context) async {
    ...
    if (!AuthUtils().isValidPassword(passwordController.text)) {
      passwordErrorText =
          Translations.of(context)!.text('textfield.error.password-policy');
      notifyListeners();
      return;
    }
    if (passwordController.text != passwordRepeatController.text) {
      passwordRepeatErrorText = Translations.of(context)!
          .text('textfield.error.mismatched-passwords');
      notifyListeners();
      return;
    }
    ...
    await GetIt.I
      ...
      if (success) {
        ...
        FlashMessageFactory.showFlashMessage(
            ...
            title: Translations.of(context)!
                .text('snackbar.password-reset.success'));
      } else {
        FlashMessageFactory.showFlashMessage(
            context: context,
            type: FlashMessageType.error,
            title:
                Translations.of(context)!.text('snackbar.code-input.failure'));
      }
    });
  }
