...
class KPIManager extends ChangeNotifier {
  ...
  initKPIs() {
    kpis = List.from(databaseRepository.getKPIsByRoute(currentRoute));
  ...
  updatePosition(int oldIndex, int newIndex) {
    // KPI with old index
    KeyPerformanceIndex movedKPI = kpis
        .firstWhere((k) => k.position == oldIndex && k.route == currentRoute);
    ...
    if (newIndex > oldIndex) {
      ...
      for (var kpi in kpis) {
        if (kpi != movedKPI &&
            kpi.position > oldIndex &&
            kpi.position <= newIndex) {
          kpi.position--;
        }
      }
    } else {
  ...
  toggleDisabled(KeyPerformanceIndex kpi) {
    KeyPerformanceIndex updateKpi = kpis.firstWhere((k) =>
        k.name == kpi.name &&
        k.category == kpi.category &&
        k.route == kpi.route);
