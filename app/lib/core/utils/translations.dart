...
import 'package:spend/core/extensions/string_extension.dart';
...
class Translations {
  ...
  String text(String key) {
    if (_localizedValues?.containsKey(key) ?? false) {
      return _localizedValues![key].toString().isEmpty
          ? key.removeHtmlTags()
          : _localizedValues![key].toString().removeHtmlTags();
    } else {
  ...
  static Future<void> initApiTranslations() async {
    Map<String, dynamic> apiTranslations =
        await GetIt.I.get<ApiRepository>().getApiTranslations();
  ...
  static Future<Translations> load(Locale locale) async {
    ...
    String? storedLanguageCode =
        prefs.getString(Constants.kSharedPrefLanguageKey);
    ...
    for (String filePath in translationPaths) {
      mergedTranslations.addAll(json.decode(// create map
  ...
  static Map<String, dynamic> _getApiMapByLocale(Locale locale) {
    switch (locale.languageCode.toLowerCase()) {
      ...
      default:
        throw Exception(
            '[Translations] _getApiMapByLocale: Invalid locale provided: $locale');
    }
  }
}
...
class TranslationsDelegate extends LocalizationsDelegate<Translations> {
  ...
  @override
  bool isSupported(Locale locale) => GetIt.I
      .get<AppLocalization>()
      .supportedLanguages
