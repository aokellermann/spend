...
import 'package:spend/presentation/components/auth/control_elements/code_digit_field.dart';
...
class CodeVerificationModal extends StatefulWidget {
  ...
  const CodeVerificationModal(
      {super.key, required this.email, required this.verifyCallback});
...
class _CodeVerificationModalState extends State<CodeVerificationModal> {
  ...
  void initState() {
    ...
    _codeControllers = List.generate(
        AuthConst.kCodeDigits, (index) => TextEditingController());
                ...
                child: Column(
                  children: [
                      ...
                      children: [
                        Flexible(
                          ...
                          child: Text(
                            Translations.of(context)!
                                .text('modal.code-verification.title'),
                            maxLines: 2,
                        ...
                        IconButton(
                          icon: const Icon(Icons.close_rounded,
                              size: AuthConst.kAuthModalCloseButtonSize,
                        ...
                        children: [
                          TextSpan(
                              text: Translations.of(context)!.text(
                                  'modal.code-verification.instructions.code-sent-to')),
                          TextSpan(
                            text: widget.email,
                            style: AuthConst.kAuthModalInfoTextStyle.copyWith(
                                color: ColorPalette.disabledButtonTextColor),
                          ),
                          const TextSpan(
                            text: '\n\n',
                          ),
                          TextSpan(
                              text: Translations.of(context)!.text(
                                  'modal.code-verification.instructions.code-input')),
                        ],
                      ),
                    ),
                    const Spacer(flex: 1),
                        ...
                        await GetIt.I
                          ...
                          if (success) {
                            FlashMessageFactory.showFlashMessage(
                                ...
                                title:
                                    '${Translations.of(context)!.text('snackbar.code-resend.success')} ${widget.email}');
                          } else {
                            FlashMessageFactory.showFlashMessage(
                                context: context,
                                type: FlashMessageType.error,
                                title: Translations.of(context)!
                                    .text('snackbar.code-resend.failure'));
                          }
                        });
                      },
                    ),
                    ...
                    ValueListenableBuilder(
                        valueListenable:
                            _registrationService.isLoadingConfirmation,
                        builder: (context, loading, _) {
                          return PrimaryButton(
                            title: Translations.of(context)!
                                .text('button.title.verify-code'),
                            callback: () {
                              _registrationService.isLoadingConfirmation.value =
                                  true;
                              widget.verifyCallback(AuthUtils()
                                  .getCodeByControllers(_codeControllers));
                            },
                            isLoading: loading,
                          );
                        }),
