...
import 'package:spend/presentation/auth/password_reset/footers/password_reset_confirmation_footer.dart';
  ...
  Widget build(BuildContext context) {
    return AuthScaffold(
      header: Header(
          ...
          showSwissLogo: true),
      footer:
          PasswordResetConfirmationFooter(codeControllers: _codeControllers),
      body: [
        Padding(
                ...
                children: [
                  TextSpan(
                    text: Translations.of(context)!.text(
                        'password-reset.confirmation.instructions.password-input'),
                  ),
                  TextSpan(
            ...
            builder: (_, error, __) {
              return PasswordTextField(
                label: Translations.of(context)!
                    .text('textfield.label.password-repeat'),
                controller:
                    context.read<PasswordService>().passwordRepeatController,
                errorText: error,
                inputAction: TextInputAction.done,
                focusNode:
                    context.read<PasswordService>().passwordRepeatFocusNode,
                hintText: Translations.of(context)!
                    .text('textfield.hint.password-repeat'),
              );
            }),
        Gaps.vSpacingS,
        CodeDigitField(
          ...
          resendCodeCallback: () async => await GetIt.I
              .get<AuthRepository>()
              .resetPassword(
                  context.read<PasswordService>().emailController.text),
          hasSubtitle: true,
        ),
        Gaps.vSpacingS,
      ],
