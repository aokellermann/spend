...
import 'package:spend/amplifyconfiguration.dart';
import 'package:spend/core/resources/constants.dart';
import 'package:spend/domain/abstractions/auth_repository_abstraction.dart';
...
class AuthRepository extends AuthRepositoryAbstraction {
  ...
  Future<bool> signUpUser({
    ...
    try {
      ...
      return authSignUpStep == AuthSignUpStep.done ||
          authSignUpStep == AuthSignUpStep.confirmSignUp;
  ...
  Future<AuthSignInStep> _handleSignInResult(
      SignInResult result, String email) async {
  ...
  Future<String?> getAccessToken() async {
    try {
      ...
      final sub = result.userSubResult.value;
      final accesstoken = result.userPoolTokensResult.value.accessToken.raw;
      safePrint("Current user's ID: $sub");
  ...
  Future<String?> getUserEmail() async {
    try {
      final result = await Amplify.Auth.fetchUserAttributes();
      return result
          .firstWhere((u) => u.userAttributeKey == AuthUserAttributeKey.email)
          .value;
    } on AuthException catch (e) {
  ...
  Future<AuthResetPasswordStep> _handleResetPasswordResult(
  ...
  Future<bool> updateUserEmail({
    ...
    try {
      ...
      safePrint(
          '[updateUserEmail] Error updating user attribute: ${e.message}');
      return false;
    }
  }

  @override
  Future<bool> updateUsername({required String newEmail}) async {
    try {
      final result = await Amplify.Auth.updateUserAttribute(
      ...
      safePrint(
          '[_updateUsername] Error updating user attribute: ${e.message}');
      return false;
    }
  }
  ...
  Future<bool> verifyEmailUpdate(
      {required String newEmail, required String confirmationCode}) async {
    try {
