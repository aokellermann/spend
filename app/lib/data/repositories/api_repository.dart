...
import 'package:spend/core/mixins/api_repository_mixin.dart';
...
import 'package:spend/domain/abstractions/api_repository_abstraction.dart';
...
class ApiRepository extends ApiRepositoryAbstraction with ApiRepositoryMixin {
  ...
  Future<Map<String, dynamic>> getApiTranslations() async {
    final response =
        await GetIt.I.get<ApiBaseHelper>().get('dynamic-translations');
    return response;
  }
  ...
  Future<List<KeyPerformanceIndex>> getEnergyKPIs() async {
    ...
    final List<KeyPerformanceIndex> newKPIs =
        KeyPerformanceIndexResponse.fromJson(
      json: response,
  ...
  Future<List<KeyPerformanceIndex>> getWeatherKPIs() async {
    final response =
        await GetIt.I.get<ApiBaseHelper>().get('v2/dashboard/wetter');
    final List<KeyPerformanceIndex> newKPIs =
        KeyPerformanceIndexResponse.fromJson(
                json: response,
  ...
  Future<List<KeyPerformanceIndex>> getOverviewKPIs() async {
    ...
    final List<KeyPerformanceIndex> overviewDbKpis =
        databaseRepository.getOverviewKPIs();
    final Set<KPICategory> overviewCategories =
        overviewDbKpis.map((k) => k.category).toSet();
    ...
    List<KeyPerformanceIndex> newKPIs = allApiKpis
        .map((k) {
  ...
  @override
  Future<List<KeyPerformanceIndex>> getKPIsByCategory(
      KPICategory category) async {
