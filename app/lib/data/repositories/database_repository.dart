...
import 'package:spend/domain/abstractions/database_repository_abstraction.dart';
...
class DatabaseRepository extends DatabaseRepositoryAbstraction {
  ...
  List<KeyPerformanceIndex> getEnergyKPIs() {
    return kpiBox.values
        .where((k) =>
            k.category == KPICategory.energy &&
            k.route == NavigationRoute.energy)
        .toList()
  ...
  List<KeyPerformanceIndex> getGasKPIs() {
    return kpiBox.values
        .where((k) =>
            k.category == KPICategory.gas && k.route == NavigationRoute.gas)
        .toList()
      ..sort((a, b) => a.position.compareTo(b.position));
  }
  ...
  List<KeyPerformanceIndex> getPriceKPIs() {
    return kpiBox.values
        .where((k) =>
            k.category == KPICategory.price && k.route == NavigationRoute.price)
        .toList()
      ..sort((a, b) => a.position.compareTo(b.position));
  }
  ...
  List<KeyPerformanceIndex> getWeatherKPIs() {
    return kpiBox.values
        .where((k) =>
            k.category == KPICategory.weather &&
            k.route == NavigationRoute.weather)
        .toList()
      ..sort((a, b) => a.position.compareTo(b.position));
  }
  ...
  void saveChanges(List<KeyPerformanceIndex> kpis, NavigationRoute route,
    ...
    if (route == NavigationRoute.overview &&
        kpis.length != getOverviewKPIs().length) {
  ...
  void addFavorite(KeyPerformanceIndex kpi, BuildContext context) {
    ...
    final KeyPerformanceIndex newFavorite =
        kpi.copyWith(route: NavigationRoute.overview, position: 0);
  ...
  void removeFavorite(KeyPerformanceIndex kpi, BuildContext context) {
    final deleteKPI = getOverviewKPIs().firstWhere(
      (k) =>
          k.name == kpi.name &&
          k.category == kpi.category &&
  ...
  void removeUnmappedKPIs() {
    ...
    final allStoredKPIsAreMapped = allStoredKPIs.every((stored) =>
        allMappedKPIs.any((mapped) =>
            mapped.name == stored.name && mapped.category == stored.category));
  ...
  List<KeyPerformanceIndex> getUnmappedKPIs() {
    ...
    return allStoredKPIs
        .where((stored) => !allMappedKPIs.any((mapped) =>
            mapped.name == stored.name && mapped.category == stored.category))
        .toList();
  }
}
